...
def evaluation(X, y, model, n_preds=10, random=True, show_graph=True):
    ...
    if show_graph == True:
        ...
        plt.legend(loc=1)
        plt.title('Return Prediction', fontSize=15)
        plt.show()
...
def bb_trading(stock, budget=100000, u=0.003, l=0.003, show_graph=True, show_return=True):
    ...
    for i in range(len(stock)):
        ...
        # Buy
        if (today.Close < today.ma_30) and (abs(1 - today.Close / today.Lower_band) < u):
            if money > 0:
        ...
        # Sell        
        if (today.Close > today.ma_30) and (abs(1 - today.Upper_band / today.Close) < l):
            if money == 0 and stock_amt > 0:
...
def macd_trading(stock, budget=100000, show_graph=True, show_return=True):
